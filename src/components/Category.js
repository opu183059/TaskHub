...
const Category = ({ tasks, onDrop, category }) => {
    ...
    return (
        <div ref={drop} className={`${category === "todo" && "bg-orange-100"} ${category === "inProgress" && "bg-yellow-100"} ${category === "done" && "bg-green-100"} rounded-lg min-h-20 md:min-h-screen`}>
            <h2 className={`${category === "todo" && "bg-orange-600"} ${category === "inProgress" && "bg-yellow-600"} ${category === "done" && "bg-green-600"} p-2 md:p-5 rounded-t-lg uppercase text-white font-bold`}>{category === "todo" && "To D0"}{category === "inProgress" && "In Progress"}{category === "done" && "Done"}</h2>
